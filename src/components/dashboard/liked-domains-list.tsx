...
import { ThumbsUp, ExternalLink, Trash2, Calendar, Lightbulb } from 'lucide-react';
import { Button, Badge } from '@/components/ui';

interface LikedDomain {
  likeId: string;
  createdAt: string;
  id: string;
  domain_name: string;
...
interface LikedDomainsListProps {
  domains: LikedDomain[];
}

export function LikedDomainsList({ domains: initialDomains }: LikedDomainsListProps) {
  const [domains, setDomains] = useState(initialDomains);
  const [removing, setRemoving] = useState<string | null>(null);

  const handleUnlike = async (likeId: string, listingId: string) => {
    setRemoving(likeId);
    ...
    try {
      ...
      if (response.ok) {
        setDomains(prev => prev.filter(d => d.likeId !== likeId));
      }
    } catch (error) {
      console.error('Failed to unlike domain:', error);
    } finally {
      setRemoving(null);
    }
  };
  ...
  return (
    <div className="divide-y divide-gray-100">
      {domains.map((domain) => {
        ...
        return (
          <div key={domain.likeId} className="py-3 flex items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <Button
                variant="ghost"
                size="sm"
                onClick={() => handleUnlike(domain.likeId, domain.id)}
                disabled={removing === domain.likeId}
